...
const BusinessForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Basic validations
    if (!formData.name || !formData.email || !formData.phone || !formData.businessName || !formData.businessType || !formData.coverageAmount) {
    ...
    if (!/^\d+(\.\d{1,2})?$/.test(formData.coverageAmount)) {
